...
(function() {
  ...
  (extendNative = function(to, from) {
    ...
    for (key in from.prototype)
      try {
        (val = from.prototype[key]),
          null == to[key] && "function" != typeof val
    ...
    (defaultOptions = {
      checks: {
        xhr: {
          url: function() {
            return "/logoicon.png?_=" + new Date().getTime();
          },
          timeout: 5e3,
          type: "HEAD"
        },
    ...
    (grab = function(obj, key) {
      ...
      for (
        cur = obj, parts = key.split("."), i = j = 0, len = parts.length;
        j < len && ((part = parts[i]), "object" == typeof (cur = cur[part]));
        i = ++j
      );
    ...
    (Offline.getOption = function(key) {
      ...
      return (
        (val =
          null != (ref = grab(Offline.options, key))
            ? ref
            : grab(defaultOptions, key)),
    ...
    (Offline.markUp = function() {
      if ((Offline.trigger("confirmed-up"), "up" !== Offline.state))
        return (Offline.state = "up"), Offline.trigger("up");
    }),
    (Offline.markDown = function() {
      if ((Offline.trigger("confirmed-down"), "down" !== Offline.state))
        return (Offline.state = "down"), Offline.trigger("down");
    }),
    (handlers = {}),
    (Offline.on = function(event, handler, ctx) {
      var e, events, j, len, results;
      if (((events = event.split(" ")), events.length > 1)) {
        for (results = [], j = 0, len = events.length; j < len; j++)
          (e = events[j]), results.push(Offline.on(e, handler, ctx));
        return results;
      }
      return (
        null == handlers[event] && (handlers[event] = []),
    ...
    (Offline.off = function(event, handler) {
      ...
      if (null != handlers[event]) {
        if (handler) {
          for (i = 0, results = []; i < handlers[event].length; )
            (ref = handlers[event][i]),
              ref[0],
              (_handler = ref[1]),
              _handler === handler
    ...
    (Offline.trigger = function(event) {
      ...
      if (null != handlers[event]) {
        for (
          ref = handlers[event].slice(0), results = [], j = 0, len = ref.length;
          j < len;
          j++
        )
          (ref1 = ref[j]),
        ...
        null === xhr.onprogress
          ? ((_onerror = xhr.onerror),
            ...
            (xhr.ontimeout = function() {
              return (
                onDown(),
                "function" == typeof _ontimeout
                  ? _ontimeout.apply(null, arguments)
                  : void 0
              );
            }),
            (_onload = xhr.onload),
          ...
          : ((_onreadystatechange = xhr.onreadystatechange),
            (xhr.onreadystatechange = function() {
              return (
                4 === xhr.readyState
                  ? checkStatus()
                  : 0 === xhr.readyState && onDown(),
                "function" == typeof _onreadystatechange
    ...
    (Offline.checks.xhr = function() {
      ...
      (xhr = new XMLHttpRequest()),
        (xhr.offline = !1),
        xhr.open(
          Offline.getOption("checks.xhr.type"),
          Offline.getOption("checks.xhr.url"),
          !0
        ),
        null != xhr.timeout &&
    ...
    (Offline.onXHR = function(cb) {
      ...
      if (
        ((monitorXHR = function(req, flags) {
          ...
          return (
            (_open = req.open),
            (req.open = function(type, url, async, user, password) {
              return (
                cb({
                  type: type,
                  url: url,
    ...
    (init = function() {
      if (
        (Offline.getOption("interceptRequests") &&
          Offline.onXHR(function(arg) {
            var xhr;
            if (((xhr = arg.xhr), !1 !== xhr.offline))
              return checkXHR(xhr, Offline.markUp, Offline.confirmDown);
          }),
        Offline.getOption("checkOnLoad"))
      )
    ...
    (rc = Offline.reconnect = {}),
      ...
      (reset = function() {
        ...
        return (
          ...
          (rc.state = "inactive"),
          (rc.remaining = rc.delay =
            null != (ref = Offline.getOption("reconnect.initialDelay"))
              ? ref
              : 3)
        );
      }),
      (next = function() {
        ...
        return (
          (delay =
            null != (ref = Offline.getOption("reconnect.delay"))
              ? ref
              : Math.min(Math.ceil(1.5 * rc.delay), 3600)),
          (rc.remaining = rc.delay = delay)
        );
      }),
      (tick = function() {
        if ("connecting" !== rc.state)
          return (
            (rc.remaining -= 1),
    ...
    (held = []),
      ...
      (holdRequest = function(req) {
        if (!1 !== Offline.getOption("requests"))
          return (
            ...
            "down" !== Offline.state && (waitingOnConfirm = !0),
            held.push(req)
          );
      }),
      (makeRequest = function(arg) {
        ...
        if (
          ...
          (body = arg.body),
          !1 !== Offline.getOption("requests"))
        ) {
          xhr.abort(),
            xhr.open(type, url, !0, user, password),
            (ref = xhr.headers);
          for (name in ref) (val = ref[name]), xhr.setRequestHeader(name, val);
          return (
            xhr.mimeType && xhr.overrideMimeType(xhr.mimeType), xhr.send(body)
          );
        }
      }),
      (clear = function() {
      ...
      (flush = function() {
        ...
        if (!1 !== Offline.getOption("requests")) {
          for (
            ...
            (request = held[i]),
              (url = request.url.replace(/(\?|&)_=[0-9]+/, function(
                match,
                chr
              ) {
                return "?" === chr ? chr : "";
              ...
              Offline.getOption("deDupBody")
                ? ((body = request.body),
                  (body =
                    "[object Object]" === body.toString()
                      ? JSON.stringify(body)
                      : body.toString()),
                  (requests[
                    request.type.toUpperCase() + " - " + url + " - " + body
                  ] = request))
                : (requests[
                    request.type.toUpperCase() + " - " + url
                  ] = request);
          ...
          return (
            ...
            Offline.onXHR(function(request) {
              ...
              if (
                ...
                return null === xhr.onprogress
                  ? (xhr.addEventListener("error", hold, !1),
                    xhr.addEventListener("timeout", hold, !1))
                  : ((_onreadystatechange = xhr.onreadystatechange),
                    (xhr.onreadystatechange = function() {
                      return (
                        0 === xhr.readyState
  ...
  function() {
    ...
    for (ref = ["up", "down"], i = 0, len = ref.length; i < len; i++) {
      ...
      try {
        simulate =
          document.querySelector("script[data-simulate='" + state + "']") ||
    ...
    simulate &&
      ...
      null == (base = Offline.options).checks && (base.checks = {}),
  ...
  function() {
    ...
    (TEMPLATE =
      ...
      (flashClass = function(name, time) {
        return (
          addClass(name),
          null != flashTimeouts[name] && clearTimeout(flashTimeouts[name]),
      ...
      (roundTime = function(sec) {
        ...
        for (unit in units)
          if (((mult = units[unit]), sec >= mult))
            return (val = Math.floor(sec / mult)), [val, unit];
        return ["now", ""];
      }),
      (render = function() {
        var button, handler;
